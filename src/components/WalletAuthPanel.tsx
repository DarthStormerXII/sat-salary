...
export function WalletAuthPanel({
  ...
  return (
    <div className="wallet-auth-card" aria-label="Wallet authentication">
      ...
      <p>
        Live Mezo payroll testing requires a real EVM wallet account, Mezo
        Testnet chain
        {` `}31611, and a signed operator message. The app will not mark a
        wallet connected without a returned signature.
      </p>
      <button
      ...
      {isConnected ? (
        <div
          className="wallet-auth-status wallet-auth-status--connected"
          data-testid="wallet-auth-status"
        >
      ...
      {isBlocked ? (
        <div
          className="wallet-auth-status wallet-auth-status--blocked"
          data-testid="wallet-auth-status"
        >
